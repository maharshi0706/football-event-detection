...
#         self.writer = SummaryWriter(log_dir=config.LOG_DIR / "mvitv2")
#         self.best_acc = 0.0
...
#         for step, (videos, labels) in enumerate(tqdm(self.train_loader, desc=f"Epoch {epoch+1}")):
#             videos, labels = videos.to(config.DEVICE), labels.to(config.DEVICE)

#             with autocast(device_type="cuda", dtype=torch.float16):
#                 logits = self.model(videos)  # timm returns (B, num_classes)
...
import torch
import torch.nn as nn
from torch.amp import autocast, GradScaler  # Modern import (fixes deprecation)
from torch.optim import AdamW
from torch.optim.lr_scheduler import CosineAnnealingLR
from torch.utils.tensorboard import SummaryWriter
from sklearn.metrics import accuracy_score, f1_score
